...
OpencvWrapperFrame::OpencvWrapperFrame() : child_(nullptr), pop_(nullptr) {}
OpencvWrapperFrame::~OpencvWrapperFrame() {}

LRESULT OpencvWrapperFrame::HandleMessage(UINT msg, WPARAM w_param,
                                          LPARAM l_param) {
  switch (msg) {
    case WM_CREATE: {
...
bool OpencvWrapperFrame::OnCommand(WPARAM w_param, LPARAM l_param,
                                   LRESULT& l_res) {  // NOLINT
  switch (LOWORD(w_param)) {
    case ID_HELP_ABOUT: {
      std::wstring version = Utf8ToUtf16(OPENCV_WRAPPER::GetVersion());
      ::MessageBox(GetHwnd(), version.c_str(), TEXT("OpencvWrapperApp"),
                   MB_ICONINFORMATION);
    } break;
    case ID_VIEW_EXIT: {
      Close();
    ...
    case ID_VIEW_POP: {
      ...
      // pop window
      pop_ = new OpencvWrapperPopWindow();
      ...
      left_top.x += width / 2;
      left_top.y += height / 2;
      pop_->Create(nullptr, TEXT("OpencvWrapperPopWindow"), pop_style, 0,
...
void OpencvWrapperFrame::Init() {
  // process child window
  child_ = new OpencvWrapperRichEditWindow();
