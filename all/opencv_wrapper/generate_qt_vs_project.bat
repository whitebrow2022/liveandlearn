...
cmake -S "%bat_dir%\apps\opencv_wrapper_app\qt\OpencvWrapperApp" -B "%vs_qt_dir%" -G "Visual Studio 17 2022" -A x64 -T v143 -DWINDOWS:BOOL=ON -DCMAKE_PREFIX_PATH=%qt_msvc_path% -DQT_MSVC_DIR="%qt_msvc_path%" -DBUILD_opencv_dnn=OFF -DBUILD_TESTS=OFF -DBUILD_EXAMPLES=OFF -DBUILD_opencv_apps=OFF -DBUILD_PERF_TESTS=OFF -DBUILD_SHARED_LIBS=OFF -DWITH_FFMPEG=ON -DOPENCV_FFMPEG_USE_FIND_PACKAGE=ON
...
if exist %qt_app% (
  ...
  mklink "%qt_app%" "%vs_qt_dir%\opencv_wrapper_app.sln"
)
