@cls
@title Generate 'Visual Studio 17 2022' solution for OpencvWrapper mfc app
@echo off
...
echo Generate opencv_wrapper_mfc_app
set vs_app_dir=%bat_dir%\build\windows\vs2022\apps\opencv_wrapper_app
cmake -S "%bat_dir%\apps\opencv_wrapper_app\windows\opencv_wrapper_app" -B "%vs_app_dir%" -G "Visual Studio 17 2022" -A Win32 -T v143 -DWINDOWS:BOOL=ON
set cmake_generate_err=%ERRORLEVEL%
if %cmake_generate_err% NEQ 0 (
  exit /b %cmake_generate_err%
)
set vs_app_sln=%bat_dir%\vs2022_app.sln
if exist %vs_app_sln% (
  echo '%vs_app_sln%' exist
) else (
  mklink "%vs_app_sln%" "%vs_app_dir%\opencv_wrapper_app.sln"
)
