#!/bin/bash
#
# Generate 'iOS' project for OpencvWrapper.

bash_dir="$( cd -- "$(dirname "$0")" >/dev/null 2>&1 ; pwd -P )"

echo "generate ios project ..."

ios_proj_dir="$bash_dir/build/ios/apps/opencv_wrapper"
...
if ! [ -h "$app_xcworkspace" ]; then
  ln -s "$bash_dir/apps/opencv_wrapper_app/ios/OpencvWrapperApp.xcworkspace" "$app_xcworkspace"
fi
