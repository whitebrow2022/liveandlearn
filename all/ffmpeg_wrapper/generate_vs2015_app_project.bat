@cls
@title Generate 'Visual Studio 14 2015' solution for FfmpegWrapper mfc app
@echo off
...
set vs_app_dir=%bat_dir%\build\windows\vs2015\apps\ffmpeg_wrapper_app
cmake -S "%bat_dir%\apps\ffmpeg_wrapper_app\windows\ffmpeg_wrapper_app" -B "%vs_app_dir%" -G "Visual Studio 14 2015" -A Win32 -T v140 -DWINDOWS:BOOL=ON
set cmake_generate_err=%ERRORLEVEL%
if %cmake_generate_err% NEQ 0 (
  exit /b %cmake_generate_err%
)
set vs_app_sln=%bat_dir%\vs2015_app.sln
if exist %vs_app_sln% (
  echo '%vs_app_sln%' exist
) else (
  mklink "%vs_app_sln%" "%vs_app_dir%\ffmpeg_wrapper_app.sln"
)
