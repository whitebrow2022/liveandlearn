...
int TestHelloWorld(const char* src_file) {
  ...
  if (avformat_open_input(&pFormatContext, src_file, NULL, NULL) != 0) {
  ...
  if (video_stream_index == -1) {
    logging("File %s does not contain a video stream!", src_file);
    return -1;
  }
