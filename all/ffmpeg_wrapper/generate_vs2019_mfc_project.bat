...
set vs_app_dir=%bat_dir%\build\windows\vs2019\apps\ffmpeg_wrapper_mfc_app
cmake -S "%bat_dir%\apps\ffmpeg_wrapper_app\windows\FfmpegWrapperMfcApp" -B "%vs_app_dir%" -G "Visual Studio 16 2019" -A Win32 -T v142 -DWINDOWS:BOOL=ON
set cmake_generate_err=%ERRORLEVEL%
if %cmake_generate_err% NEQ 0 (
  exit /b %cmake_generate_err%
)
set vs_mfc_app_sln=%bat_dir%\vs2019_mfc_app.sln
