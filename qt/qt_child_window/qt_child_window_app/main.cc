...
void LoadTranslations(QTranslator* translator) {
  ...
  LoadTranslation(&translator[0],
                  app_path + "/../../qt_child_window_app_" + locale);
  {
    LoadTranslation(&translator[1],
                    app_path + "/../qt_child_window_app_" + locale);
    LoadTranslation(&translator[2], app_path + "/qt_child_window_app_" + locale);
  }
}
...
int main(int argc, char* argv[]) {
  ...
  QString log_path = QString("%1/qt_child_window_app_%2.log").arg(log_dir)
                         .arg(curr_time_str);
  QDir log_qdir(log_dir);
  if (!log_qdir.exists()) {
    log_qdir.mkpath(".");
  }
  QT_CHILD_WINDOW_BASE::InitLog(log_path.toStdString().c_str());
  log_info << "qt_child_window_app start:";
  // multiple langs
  // QTranslator translator[3];
  // LoadTranslations(translator);

  QtChildWindowFrame w;
  QObject::connect(&a, &QApplication::aboutToQuit, []() {
    // exit app
    log_info << "qt_child_window_app about to quit!";
  });
  w.show();
  auto ret = a.exec();
  QT_CHILD_WINDOW_BASE::UnInitLog();
  return ret;
}
